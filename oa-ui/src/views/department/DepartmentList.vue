...
function handleDelete(row) {
  ...
  if (children.length) {
    ElMessage.warning('该项目部下有子项目部，无法删除')
    return
  }
  const deptUsers = users.filter(u => u.deptId === row.id)
  if (deptUsers.length) {
    ElMessage.warning(`该项目部下有 ${deptUsers.length} 名员工，无法删除`)
    return
  }
  ElMessageBox.confirm(`确定要删除项目部「${row.name}」吗？`, '提示', { type: 'warning' })
    .then(() => {
...
const rules = {
  name: [{ required: true, message: '请输入项目部名称', trigger: 'blur' }],
  code: [{ required: true, message: '请输入项目部编码', trigger: 'blur' }]
}
</script>
  ...
  <div class="page-shell">
    ...
    <el-card class="search-card">
      <el-form :model="searchForm" inline class="toolbar-form">
        <el-form-item label="关键字">
          <el-input v-model="searchForm.keyword" placeholder="项目部名称/编码" clearable style="width:200px" />
        </el-form-item>
    ...
    <el-card class="table-card">
      <div class="page-header">
        <div>
          <h2>项目部列表</h2>
          <p class="page-subtitle">管理组织架构中的所有项目部</p>
        </div>
        <el-button type="primary" :icon="Plus" round @click="openAdd(null)">新增项目部</el-button>
      </div>
      <div class="table-wrap">
        <el-table :data="filteredTree" row-key="id" class="data-table" style="width:100%"
          ...
          <el-table-column prop="name" label="项目部名称" width="240" fixed="left" />
          <el-table-column prop="code" label="项目部编码" width="130" />
          <el-table-column prop="leader" label="负责人" width="100" />
          <el-table-column prop="parentId" label="上级项目部ID" width="110" align="center">
            <template #default="{ row }">{{ row.parentId || '顶级项目部' }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="260" fixed="right" align="center">
            <template #default="{ row }">
              <el-button type="primary" link :icon="Plus" @click="openAdd(row.id)">添加子项目部</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="560px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="上级项目部">
          <el-select v-model="form.parentId" placeholder="无（顶级项目部）" clearable style="width:100%">
        ...
        <el-form-item label="项目部编码" prop="code">
